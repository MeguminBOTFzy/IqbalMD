const { default: makeWASocket, BufferJSON, MessageType, MessageOptions, Mimetype, generateWAMessageFromContent, downloadContentFromMessage, proto, areJidsSameUser, getContentType, delay } = require('@adiwajshing/baileys')
...
exports.smsg = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        m.bal = m.message[m.mtype]
        if (m.mtype === 'ephemeralMessage') {
            exports.smsg(iqbl, m.bal)
            m.mtype = m.bal.mtype
            m.bal = m.bal.bal
        }
        let quoted = m.quoted = m.bal.contextInfo ? m.bal.contextInfo.quotedMessage : null
        m.mentionedJid = m.bal.contextInfo ? m.bal.contextInfo.mentionedJid : []
        if (m.quoted) {
            ...
            m.quoted.id = m.bal.contextInfo.stanzaId
			m.quoted.chat = m.bal.contextInfo.remoteJid || m.chat
            m.quoted.isBaileys = m.quoted.id ? m.quoted.id.startsWith('BAE5') && m.quoted.id.length === 16 : false
			m.quoted.sender = m.bal.contextInfo.participant.split(":")[0] || m.bal.contextInfo.participant
			m.quoted.fromMe = m.quoted.sender === (conn.user && conn.user.id)
            m.quoted.text = m.quoted.text || m.quoted.caption || ''
			m.quoted.mentionedJid = m.bal.contextInfo ? m.bal.contextInfo.mentionedJid : []
    ...
    if (m.bal.url) m.download = () => downloadMediaMessage(m.bal)
    m.text = (m.mtype == 'listResponseMessage' ? m.bal.singleSelectReply.selectedRowId : '') || m.bal.text || m.bal.caption || m.bal || ''
    /**
